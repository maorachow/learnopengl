...
#include "chunkmanager.cpp"
	...
	//static std::vector < Chunk>Chunks;//静态map
	...
	bool Chunk::CheckisInView(glm::vec3 position,glm::vec3 normal) {//视锥剔除
	//	Plane plane = Plane{ normal,glm::length(position-normal)};
		...
		for (int i = 0; i < deltas.size(); i++) {
		if (glm::dot(normal,deltas[i])>0) {//摄像机前方chunk不剔除，后方剔除
			return true;
		}
		}
	...
	/*	glm::mat4 model = glm::mat4(1.0f);
		...
		for (int i = 0; i < boundingBoxPoints.size(); i++) {
			boundingBoxPoints[i] = boundingBoxPoints[i] * model * view *projection;//矩阵乘向量
		}
		bool result = false;
		for (int i = 0; i < boundingBoxPoints.size(); i++) {
